...
def fetch_issue_comments(repo_owner: str, repo_name: str, issue_number: int):
    return comment_service.fetch_issue_comments(
        repo_owner,
        repo_name,
        issue_number)
...
@router.get("/pull-request",
            response_model=List[comment_schema.PullReqCommentResp])
def fetch_pullreq_comments(repo_owner: str,
                           repo_name: str,
                           pullreq_number: int):
    return comment_service.fetch_pullreq_comments(repo_owner,
                                                  repo_name,
                                                  pullreq_number)
