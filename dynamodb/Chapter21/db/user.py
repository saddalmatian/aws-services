...
def get_user(username:str):
    response = table.query(
        KeyConditionExpression=Key("PK").eq("ACCOUNT#"+username) & Key("SK").eq("ACCOUNT#"+username),
...
def get_org(orgname:str):
    response = table.query(
        KeyConditionExpression=Key("PK").eq("ACCOUNT#"+orgname) & Key("SK").eq("ACCOUNT#"+orgname),
        FilterExpression="#type = :type",
        ExpressionAttributeNames={
            "#type":"Type"
        },
...
def get_user_repo(username:str):
    response = table.query(
        IndexName="GSI3",
        KeyConditionExpression=Key("GSI3PK").eq("ACCOUNT#"+username) &Key("GSI3SK").begins_with("#"),
    )
    return response["Items"]
...
def get_org_repo(orgname:str):
    response = table.query(
        IndexName="GSI3",
        KeyConditionExpression=Key("GSI3PK").eq("ACCOUNT#"+orgname) &Key("GSI3SK").begins_with("#"),
    )
    return response["Items"]